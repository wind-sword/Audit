...
class Call_index(QtWidgets.QMainWindow,Ui_indexWindow):
    ...
    def btfun5_1(self):
        p = QtWidgets.QFileDialog.getExistingDirectory(None, "选取文件夹", "C:/")
        self.lineEdit_file_3.setText(p)
    ...
    def btfun6(self):
        ...
        input16 = self.lineEdit_file.text() #文件路径

        sql = "insert into project(专报标题,报送范围,发文字号,紧急程度,秘密等级,是否公开,拟稿人,拟稿处室分管厅领导,拟稿处室,综合处编辑,综合处审核,秘书处审核,综合处分管厅领导,审计办主任,办文日期,报文内容,projectType) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',1);"%(input1,input2,input3,input4,input5,input6,input7,input8,input9,input10,input11,input12,input13,input14,input15,input16)
        self.executeSql(sql)
    ...
    def btfun6_1(self):
        ...
        input7 = self.lineEdit_file_3.text() #文件路径
    ...
    def btfun8(self):
        ...
        if row == -1:
            ...
            sql = 'select project.专报标题,project.报送范围,project.发文字号,project.紧急程度,project.秘密等级,project.是否公开,project.拟稿人,project.拟稿处室分管厅领导,project.拟稿处室,project.综合处编辑,project.综合处审核,project.秘书处审核,project.综合处分管厅领导,project.审计办主任,project.公文标题,project.领导审核意见,project.审计办领导审核意见,project.办文情况说明和拟办意见,project.projectType from project where 发文字号 =  \'%s\''%key
            data = self.executeSql(sql)
            print(data)
