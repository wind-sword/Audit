...
class Call_index(QtWidgets.QMainWindow, Ui_indexWindow):
    ...
    def showBwprocessTable(self):
        ...
        sql = "SELECT bwprocess.序号,bwprocess.流程开始时间,sendfile.发文字号,sendfile.发文标题,count(problem.序号),revfile.收文字号," \
    ...
    def searchJzProject(self):
        ...
        sql = "select * from problem_jz where 审计意见或报告文号 = '%s' " % keyword
        data = tools.executeSql(sql)
    ...
    def add_sjbg(self):
        ...
        if input_file_path_sjbg == "" and input_file_path_sjjg == "" and input_file_path_excel == "":
            ...
            if input_file_path_excel != "":
                self.importExcelProblemJz(input_file_path_excel, keyword_sjbg)
                # 清空文件名输入栏
                self.lineEdit_file_excel_2.clear()
    ...
    def importExcelProblemJz(self, path, keyword):
        # 文件路径
        path.replace('/', '\\\\')
